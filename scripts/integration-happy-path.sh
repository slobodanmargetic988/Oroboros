...
if [[ -x "${BACKEND_DIR}/.venv/bin/python" ]]; then
  ...
  echo "Missing backend virtualenv python for integration happy path harness." >&2
  echo "Run ./scripts/db-bootstrap.sh for local usage or set DEPLOY_CURRENT_LINK." >&2
  exit 1
fi

cd "${BACKEND_DIR}"
exec env INTEGRATION_HAPPY_PATH_CALLER_CWD="${CALLER_CWD}" "${PYTHON_CMD}" -m app.services.integration_happy_path_cli "$@"
